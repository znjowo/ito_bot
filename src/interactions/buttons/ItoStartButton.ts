    ...
    protected async main(): Promise<void> {
        try {
            ...
            const playerCards = new Map<
                string,
                { username: string; cardCount: number }
            >();
            ...
            Logger.info(
                `itoゲームを開始しました: ${gameId} by ${this.interaction.user.username}`
            );
        } catch (error) {
    ...
    private async sendPlayerCards(
        ...
        try {
            for (const player of game.players) {
                ...
                // 各参加者にDMで手札を送信
                try {
                    const user = await this.interaction.client.users.fetch(
                        player.player.discordId
                    );
                    await user.send({
                    ...
                    Logger.error(
                        `手札送信エラー (${player.player.discordId}): ${sendError}`
                    );
