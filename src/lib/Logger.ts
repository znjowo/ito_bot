...
export class Logger {
    ...
    private static _messageBuffer: Array<{
        content: string;
    ...
    private static groupMessagesByColor(): Array<{
        color: ColorResolvable;
        messages: Array<{ content: string; level: string }>;
    }> {
        const groups: Map<
            string,
            {
        ...
        for (const item of this._messageBuffer) {
            ...
            groups
                .get(key)!
                .messages.push({ content: item.content, level: item.level });
        }

        return Array.from(groups.values());
    }

    private static sendGroupedMessages(group: {
        color: ColorResolvable;
        messages: Array<{ content: string; level: string }>;
    }): void {
